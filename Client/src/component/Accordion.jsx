...
const Accordion = ({ children, className }) => {
  ...
  return (
    <div className={`flex flex-col bg-white rounded-2xl shadow-md p-2 py-4 sm:p-4 ${isExpanded ? "closed" : ""} className`}>
      <div className="flex flex-row justify-between items-center" onClick={() => setIsExpanded((i) => !i)}>
        ...
        <svg className={`h-8 sm:h-14 icon ${isExpanded ? "icon-expanded" : ""}`} viewBox="0 0 24 24">
          <path d="M16.59 8.59 12 13.17 7.41 8.59 6 10l6 6 6-6z" />
        </svg>
      </div>
      {isExpanded && <div className="p-1 sm:p-4 mt-4 sm:mt-0">{content}</div>}
    </div>
  );
};
