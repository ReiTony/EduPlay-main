...
import { TeacherForgotSchemaCode } from "../SchemaValidation";
import { useNavigate } from "react-router-dom";

function Teacher_Enter_Code() {
    const navigate = useNavigate();
  const { values, errors, handleBlur, handleChange, handleSubmit, touched } = useFormik({
    initialValues: { code: "" },
    validationSchema: TeacherForgotSchemaCode,
    onSubmit: (values) => onSubmit(values, navigate),
  });

  const onSubmit = (values) => {
    navigate("/Teacher_Reset_Password");
  };
          ...
          <div>
            ...
            <div>
              <form action="" onSubmit={handleSubmit}>
                ...
                <p className="mt-2 text-2xl font-kumbh">Enter the code sent to your email.</p>
                <button className="w-[50%] font-sourceSans3 text-center rounded-full  p-4 mt-8 text-4xl bg-black shadow-lg hover:shadow-green-400 text-white placeholder-white font-bold" type="submit">
                  Send Code
                </button>
              </form>
            </div>
...
export default Teacher_Enter_Code;
