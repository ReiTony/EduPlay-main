...
function Teacher_CreateAssessment() {
  ...
  const handleSave = async (questions) => {
    ...
    axios.post(`${import.meta.env.VITE_API}teacher/createassessment`, { title, gradeLevel, answers, questions });
  };
        ...
        {stepNumber === 0 && (
          <div className="flex flex-col mx-auto gap-5" style={{ maxWidth: "960px", width: "100%" }}>
            ...
            <div className="flex flex-row items-center gap-2 text-2xl">
              ...
              <input className="flex-grow text-2xl rounded-full px-5 py-1 border-2 border-black" type="number" value={gradeLevel} onChange={(e) => setGradeLevel(e.target.value)} placeholder="Grade Level" />
            </div>
            <div className="flex flex-row items-center gap-2 text-2xl">
              <label>Module Number: </label>
              <input className="flex-grow text-2xl rounded-full px-5 py-1 border-2 border-black" type="number" value={moduleNumber} onChange={(e) => setModuleNumber(e.target.value)} placeholder="Module Number" />
            </div>
            <button className="bg-[#282424] rounded-full shadow-md text-2xl text-white px-8 py-2 my-6 mx-auto hover:brightness-95" onClick={() => setStepNumber((i) => i + 1)}>
              PROCEED
            </button>
          </div>
        )}
